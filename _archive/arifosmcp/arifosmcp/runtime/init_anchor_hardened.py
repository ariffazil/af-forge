...
class HardenedInitAnchor:
    ...
    async def init(
        ...
        **kwargs: Any,
        ...
        anchor_id = f"anch-{secrets.token_hex(8)}"
        state_hash = hashlib.sha256(
            f"{session_id}:{declared_name_norm}:{anchor_id}".encode()
        ).hexdigest()
...
Begin reasoning with this constitutional grounding. Flag any paradox, uncertainty, or coherence violation immediately."""
        ...
        return ToolEnvelope(
            ...
            payload={
                ...
                # L3 Sovereign Executor: Continuity Annex
                "continuity": {
                    "anchor_id": anchor_id,
                    "continuity_version": 1,
                },
                # L3 Sovereign Executor: Governance Annex
                "governance": {
                    "proof_status": "incomplete",
                    "policy_floors_checked": ["F12"],

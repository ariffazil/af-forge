...
# ═══════════════════════════════════════════════════════════════════════════════
# SOVEREIGN LOOP CONTRACTS (L3)
# ═══════════════════════════════════════════════════════════════════════════════


@dataclass(frozen=True)
class DecisionPacket:
    """Portable decision object output by mind, consumed by judge."""
...
@dataclass(frozen=True)
class Judgment:
    """Binding verdict output by judge, consumed by forge."""

    judgment_id: str
    verdict: str  # SEAL, HOLD, VOID, SABAR, PARTIAL
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "verdict_hash": self.compute_hash(),
        }
...
@dataclass(frozen=True)
class VaultPreRecord:
    """Immutable pre-execution vault record."""

    vault_id: str
    record_type: str = "pre_execution_judgment"
    linked_judgment_id: str = ""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "linked_judgment_id": self.linked_judgment_id,
...
@dataclass(frozen=True)
class ForgeManifest:
    """Constitutional boundary object for execution."""
    ...
    dispatch_constraints: dict[str, Any] = field(default_factory=dict)
    timestamp: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "authority_scope_hash": self.authority_scope_hash,
            "dispatch_constraints": self.dispatch_constraints,
            "timestamp": self.timestamp,
        }
...
class ExecutionRecord:
    ...
    thermodynamic_signature: dict[str, Any] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "thermodynamic_signature": self.thermodynamic_signature,
        }


@dataclass(frozen=True)
class VaultPostRecord:
    """Post-execution attestation closing the loop."""
...
@dataclass
class SovereignLoop:
    """Container for a complete sovereign execution trace."""

    session_id: str
    decision_packet: DecisionPacket | None = None
    judgment: Judgment | None = None
    vault_pre: VaultPreRecord | None = None
    manifest: ForgeManifest | None = None
    execution: ExecutionRecord | None = None
    vault_post: VaultPostRecord | None = None

    def is_closed(self) -> bool:
        return all(
            [
                self.decision_packet is not None,
    ...
    def replay_chain(self) -> list[dict[str, Any]]:
        chain = []
        if self.decision_packet:
            chain.append({"stage": "mind", "data": self.decision_packet.to_dict()})
        if self.judgment:
            chain.append({"stage": "judge", "data": self.judgment.to_dict()})
        if self.vault_pre:
            chain.append({"stage": "vault_pre", "data": self.vault_pre.to_dict()})
        if self.manifest:
            chain.append({"stage": "forge", "data": self.manifest.to_dict()})
        if self.execution:
            exec_data = (
                self.execution.to_dict()
                if hasattr(self.execution, "to_dict")
                else dict(self.execution)
            )
            chain.append({"stage": "execute", "data": exec_data})
        if self.vault_post:
            chain.append({"stage": "vault_post", "data": self.vault_post.to_dict()})
        return chain

    def compute_chain_hash(self) -> str:
        chain_data = json.dumps(self.replay_chain(), sort_keys=True, default=str)

...
class JudgmentKernel:
    ...
    def judge_apex(
        ...
        # L3: Generate judgment_id and verdict_hash
        judgment_id = f"jdg-{secrets.token_hex(8)}"
        decision_packet_id = getattr(agi_result, 'decision_packet_id', '')
        ...
        # Compute verdict_hash after construction
        verdict_data = {
            "judgment_id": verdict_result.judgment_id,
            "verdict": verdict_result.verdict,
            "decision_packet_id": verdict_result.decision_packet_id,
            "session_id": verdict_result.session_id,
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
        verdict_result.verdict_hash = hashlib.sha256(

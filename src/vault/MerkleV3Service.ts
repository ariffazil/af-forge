...
export class MerkleV3Service {
  ...
  computeRowHash(row: TelemetryRow): string {
    const content = [
      ...
      row.agent_id ?? "",
      row.ds?.toFixed(3) ?? "",
      row.peace2?.toFixed(3) ?? "",
      row.kappa_r?.toFixed(3) ?? "",
      row.shadow?.toFixed(3) ?? "",
      row.confidence?.toFixed(3) ?? "",
      row.psi_le?.toFixed(3) ?? "",
      row.verdict,
      row.witness_human?.toFixed(2) ?? "",
      row.witness_ai?.toFixed(2) ?? "",
      row.witness_earth?.toFixed(2) ?? "",
      row.qdf?.toFixed(2) ?? "",
    ].join("|");
